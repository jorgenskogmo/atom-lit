...
`;
  ...
  var Icon = class extends h3 {
    ...
    writeSprite() {
      if (!this.force && document.querySelector("#atom-iconsprite")) {
        return;
      }
      console.log(
...
`;
  ...
  var Code = class extends Atom {
    ...
    format(raw, language) {
      const text = (0, import_html_entities.decode)(dedent_default`${raw.trim()}`);
      if (language === "ATOM-CODE-EXAMPLE") {
        return {
          language,
          raw: text,
    ...
    render() {
      ...
      return ke`    
        <div class="container">
            ...
            <atom-button class="copyBtn" label="alo?" bind="clicks">
                <atom-icon slot="left" name="aperture" force=true></atom-icon>
            </atom-button>

            <!-- <button class="copyBtn" @click=${() => this.copy()}>copy <atom-icon name="aperture" style="background:#f00;"></atom-icon></button> -->
        </div>`;
    }
  };
  Code.styles = [styles6, hljsstyles];
  __decorateClass([
    r4()
