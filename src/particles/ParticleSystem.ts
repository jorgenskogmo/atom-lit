...
export class ParticleSystem {
    ...
    private _numParticles: number = 2000;
    private _attractionStrength: number = 9999;
    private _repulsionStrength: number = 0.2;
    private _repulsionRadius: number = 100;
    private _maxSpeed: number = 30;
    private _maxForce: number = 0.6;
    ...
    private _useTrails: boolean = false;
    ...
    constructor(gl: WebGL2RenderingContext) {
        ...
        this.slowColorLocation = gl.getUniformLocation(this.program, "u_slowColor")!;
        this.fastColorLocation = gl.getUniformLocation(this.program, "u_fastColor")!;
        this.arrowSizeLocation = gl.getUniformLocation(this.program, "u_arrowSize")!;
        ...
        this.trailTextureLocation = gl.getUniformLocation(this.trailProgram, "u_texture");
...
//   updateProperty<K extends keyof ParticleSystemProps>(key: K, value: ParticleSystemProps[K]): void {
...
//   }
    ...
    set fastColor(value: string | Float32Array) {
        if (typeof value === 'string') {
            if (!this.isValidHexColor(value)) {
                console.warn(`Invalid hex color: ${value}. Using default color.`);
                value = '#FF00FF'; // Default to magenta
            }
    ...
    set useTrails(value: boolean) {
        ...
        const toggleButton = document.getElementById('toggleTrails') as HTMLButtonElement;
        if (toggleButton) {
    ...
    //     this.setupTrailToggle();
    //     this.setupColorInputs();
    // }

    // Set up numeric input controls
    // private setupNumericInputs(): void {
    //     const updateValue = (id: string, property: string) => {
    //         const input = document.getElementById(id) as HTMLInputElement;
    //         input.addEventListener('input', () => {
    //             this[property] = parseFloat(input.value);
    //         });
    ...
    //     const properties = [
    //         'numParticles', 'attractionStrength', 'repulsionStrength', 'repulsionRadius',
    //         'maxSpeed', 'maxForce', 'friction', 'arrowSize'
    //     ];
    //     properties.forEach(prop => updateValue(prop, prop));
    // }

    // // Set up trail toggle button
    // private setupTrailToggle(): void {
    //     const toggleTrailsButton = document.getElementById('toggleTrails') as HTMLButtonElement;
    //     toggleTrailsButton.addEventListener('click', () => this.toggleTrails());
    //     // Initialize button text
    //     toggleTrailsButton.textContent = this.useTrails ? 'Disable Trails' : 'Enable Trails';
    // }

    // Set up color input controls
    // private setupColorInputs(): void {
    //     const updateColor = (id: string, colorProperty: string) => {
    //         const input = document.getElementById(id) as HTMLInputElement;
    //         input.addEventListener('input', () => {
    //             if (this.isValidHexColor(input.value)) {
    //                 this[colorProperty] = input.value;
    //             } else {
    //                 console.warn(`Invalid hex color: ${input.value}. Reverting to previous value.`);
    //                 input.value = this[colorProperty]; // Revert to the current valid color
    //             }
    //         });
    //         // Initialize input value
    //         input.value = this[colorProperty];
    //     };

    //     updateColor('slowColor', 'slowColor');
    //     updateColor('fastColor', 'fastColor');
    // }
    ...
    private createTrailProgram(): WebGLProgram {
        const gl = this.gl;
        const vertexShader = this.compileShader(gl.VERTEX_SHADER, this.trailVertexShaderSource);
        const fragmentShader = this.compileShader(gl.FRAGMENT_SHADER, this.trailFragmentShaderSource);
        ...
        if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
            throw new Error('Trail program linking failed: ' + gl.getProgramInfoLog(program));
        }
    ...
    private uploadParticleData(): void {
        ...
        const velocities = new Float32Array(this.particles.flatMap(p => [...p.velocity]));
    ...
    private updateParticles(): void {
        ...
        for (let i = 0; i < this.particles.length; i++) {
            ...
            if (distanceSquared > 0) {
                ...
                const attractionForce = Math.min(this.attractionStrength / distance, this.maxForce);
            ...
            for (let j = i + 1; j < this.particles.length; j++) {
                ...
                if (distSquared < this.repulsionRadius * this.repulsionRadius && distSquared > 0) {
            ...
            // Limit speed
            const speedSquared = particle.velocity[0] * particle.velocity[0] + particle.velocity[1] * particle.velocity[1];
            ...
            // Update position
            particle.position[0] = (particle.position[0] + particle.velocity[0] + width) % width;
            particle.position[1] = (particle.position[1] + particle.velocity[1] + height) % height;
        }
    ...
    private renderTrails(): void {
        ...
        const positionLocation = gl.getAttribLocation(this.trailProgram, "a_position");
